...
class PickWeekController(val pickRepository: PickRepository) {
    ...
    fun getPicksForUserByWeek(@PathVariable year:Int, @PathVariable week:Int): Set<Pick> {
        val picks: Set<Pick>  = pickRepository.getPicksForUserByWeek(1, year, week) //TODO user id
        return picks;
    }
    ...
    fun savePicksForUserByWeek(@PathVariable year:Int, @PathVariable week:Int, @RequestBody picks: Set<Pick>): Set<Pick> {
        ...
        pickRepository.savePicksForUserByWeek(1, year, week, picks) //TODO user id

        val updatedPicks: Set<Pick>  = pickRepository.getPicksForUserByWeek(1, year, week) //TODO user id
        return picks;
    }
}